...
api.get('/getUserSource.json', async (ctx) => {
    let sql = 'select source, count(source) as count from user_deal group by source order by count desc limit 10';
    let groups = await query(sql);

    sql = 'select count(distinct(source)) as total from user_deal';
    let total = await query(sql);
    total = total[0].total;
...
api.get('/getUserBySearch.json', async(ctx) => {
    ...
    if (type === 'userinfo' && username) {
        ...
        if (ret.length) {
            ...
            sql = `select introduction, keywords from user left join user_deal on user.uid=user_deal.uid where user.username='${username}'`;
            let portrait = await query(sql);
